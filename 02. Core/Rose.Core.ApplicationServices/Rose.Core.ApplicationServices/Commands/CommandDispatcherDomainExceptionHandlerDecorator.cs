using Rose.Core.Domain.Exceptions;
...
public class CommandDispatcherDomainExceptionHandlerDecorator : CommandDispatcherDecorator
{
    ...
    public override Task<CommandResult> Send<TCommand>(TCommand command)
    {
        ...
        {
            _logger.LogError(RoseEventId.DomainValidationException, ex, "Processing of {CommandType} With value {Command} failed at {StartDateTime} because there are domain exceptions.", command.GetType(), command, DateTime.Now);
    ...
    private string GetExceptionText(DomainStateException domainStateException)
    {
        ...
        _logger.LogInformation(RoseEventId.DomainValidationException, "Domain Exception message is {DomainExceptionMessage}", result);
