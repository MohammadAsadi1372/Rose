...
public abstract class CommandHandler<TCommand, TData> : ICommandHandler<TCommand, TData>
    where TCommand : ICommand<TData>
    {
        ...
        protected readonly RoseServices _RoseServices;
        protected readonly CommandResult<TData> result = new();
        [Obsolete(message: "RoseServices Dependency removed from Constructor in the next release")]
        public CommandHandler(RoseServices RoseServices)
        {
            _RoseServices = RoseServices;
        }
        ...
        {
            result.AddMessage(_RoseServices.Translator[message]);
        }
        protected void AddMessage(string message, params string[] arguments)
        {
            result.AddMessage(_RoseServices.Translator[message, arguments]);
        }
    }
